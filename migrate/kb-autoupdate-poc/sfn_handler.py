...
def stage_hit(event: dict) -> dict:
    ...
    if rc in (0, 3):
        ...
        pr = (json.load(open(os.path.join(poc(), out))).get("pr") or {}).get("url")
        return {"title": title, "status": "handled", "pr": pr}
...
def stage_finalize(event: dict) -> dict:
    ...
    prepare()
    download_run(run_id)
    pyrun("dashboard_issue.py", "--repo-for-issue", os.environ["KB_DASHBOARD_REPO"], check=False)
    ...
    return {"summary": summary, "prs": prs}
