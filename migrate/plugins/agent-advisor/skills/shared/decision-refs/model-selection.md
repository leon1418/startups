...
here. The deterministic implementation lives in `scripts/scoring.py::_select_model`; the drift
test in `scripts/test_scoring.py` locks this list against the source lifecycle file (a source
...
| multimodal | Claude Sonnet 4.6 (vision) | add Stability AI if also *generating* images |
