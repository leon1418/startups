---
_phase: clarify-pass2
_title: "Clarify (Pass 2) — Winner-specific follow-ups"
_requires_phase: clarify
_input:
  - scoring-result.json
  - answers.json
_assemble:
  _file: phases/clarify-pass2/clarify-pass2-assemble.md
_produces:
  - pass2.json
_advances_to: design
_preconditions:
  ...
  - _validate_json: scoring-result.json
    _on_failure: _unrecoverable
_postconditions:
  - _check_file_exists: pass2.json
    _on_failure: _halt_and_inform
  - _validate_json: pass2.json
    _on_failure: _halt_and_inform
  - _assert: "pass2.json has a confirmed deployment_model and the final agentcore_services list; chosen_runtime is present when the verdict was co_recommend (the runtime the user picked); tool_choices records any native-vs-gateway decisions from Step 2"
    _on_failure: _halt_and_inform
---
...
mind here. Record the final choice in `pass2.json`.
