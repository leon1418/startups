...
_postconditions:
  ...
  - _validate_json: context-signals.json
    _on_failure: _halt_and_inform
  - _assert: "context-signals.json contains only signals detected with reasonable confidence, with a _detected array; model_provider and source_platform recorded when detection succeeded"
    _on_failure: _halt_and_inform
---
...
- **Source platform**: fly.toml presence, `FLY_APP_NAME`/`FLY_PRIVATE_IP` env usage, `api.machines.dev` calls → fly.io.
...
`source_platform` (flyio | none) is not a scoring key — it records the hosting platform the
code runs on, and selects the migration-plan engine (fly.io → fly-to-aws, else gcp-to-aws) in
migration-plan.md Step 1.5. Include it whenever platform detection succeeded.
