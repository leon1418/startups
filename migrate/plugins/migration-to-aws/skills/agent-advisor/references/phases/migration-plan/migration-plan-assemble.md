...
_reads:
  - injected constraints translated from design/pass2/answers (built inline in migration-plan.md Step 2, or the fly Step-2 replacement)
  - gcp / fly global-constraints fragment (whichever engine branch fired)
_produces:
...
> **Assembler unit.** The Migration Plan phase resolves the target repo, selects
> the engine (gcp-to-aws or fly-to-aws), translates the advisor's decisions into
> the engine's constraint fields, and writes `migration-plan-injection.json`
> inline within `migration-plan.md` (Step 2; the fly branch builds it per the
> fly-constraints Step-2 replacement). It then executes the engine's
