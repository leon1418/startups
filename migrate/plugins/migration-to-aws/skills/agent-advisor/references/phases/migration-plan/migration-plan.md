...
_title: "Migration Plan — inline execution of gcp-to-aws / fly-to-aws phases"
...
_fragments:
  - _id: migration-plan-gcp-constraints
    _trigger: { _when: "source_platform is absent or not flyio (gcp / default engine)" }
    _file: phases/migration-plan/migration-plan-gcp-constraints.md
  - _id: migration-plan-fly-constraints
    _trigger: { _when: "source_platform == flyio" }
    _file: phases/migration-plan/migration-plan-fly-constraints.md
_assemble:
...
**IMPORTANT — relative path resolution table:** gcp-to-aws (or $ENGINE_BASE for fly.io sources)
...
## Step 1.5 — Select the migration engine

Read `$RUN_DIR/context-signals.json`. If `source_platform == "flyio"`: set
`$ENGINE_BASE = ${CLAUDE_PLUGIN_ROOT}/skills/fly-to-aws` and take the FLY BRANCH for the
rest of this phase — Step 2 builds the fly injection context per the table in
`references/phases/migration-plan/migration-plan-fly-constraints.md` (NOT the gcp `ai_constraints` table),
Step 2.5 loads `migration-plan-fly-constraints.md` INSTEAD of migration-plan-gcp-constraints.md,
and Phases A–E read each phase file from `$ENGINE_BASE/references/phases/...` (fly-to-aws's
own Discover → Clarify → Design → Estimate → Generate, same HANDOFF_OK discipline; fly-to-aws
files are read-only; `$MIGRATION_DIR` convention unchanged). Otherwise (`source_platform`
absent or anything else): `$ENGINE_BASE = $GCP_BASE`, continue exactly as today.

## Step 2 — Assemble the injection context

**FLY BRANCH: skip this table; build the injection per migration-plan-fly-constraints.md instead.**
...
**FLY BRANCH:** Load `references/phases/migration-plan/migration-plan-fly-constraints.md` instead of the gcp
file below, and follow everything in it for the duration of this phase.
