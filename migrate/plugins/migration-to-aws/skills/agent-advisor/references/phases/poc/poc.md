...
_preconditions:
  - _check_file_exists: [design.json, pass2.json]
    _on_failure: _unrecoverable
  - _validate_json: [design.json, pass2.json]
    _on_failure: _unrecoverable
_postconditions:
...
model_recommendation) and `$RUN_DIR/pass2.json` (confirmed deployment model + services). The
...
derive every item from `design.json` / `pass2.json` — plus, when plan-backed, from
...
3. **What gets created** — the AWS resources the POC provisions (AgentCore Runtime + the
   confirmed services from `pass2.json`, e.g. Gateway/Memory), so the user knows the blast radius
...
1. **`poc/harness.json`** — the deployable agent declaration itself (NOT a side artifact):
   ...
   `pass2.json`, session limits from the AgentCore card. Mark external wiring (Gateway
...
0. **Deployment-model fidelity gate.** Before touching any tooling, restate what is about to
   be deployed and confirm it matches `pass2.json.deployment_model` and the Step 3 branch
