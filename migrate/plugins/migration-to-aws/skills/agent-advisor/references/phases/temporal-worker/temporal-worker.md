...
_trigger: { _when: "entry_point == temporal_worker (Turn 1 trigger confirmation, or Discover detected Temporal and the user confirmed) AND phases.temporal_worker != completed" }
_input: workspace
_assemble:
...
Reached when `entry_point == temporal_worker` (Turn 1 trigger confirmation, or
...
This is a self-contained branch: it does NOT pass through Clarify / Clarify
Pass 2 / Design / Estimate / Generate, so the phase gate never applies to it.
...
Reuse the Turn 1 `$RUN_DIR` — it already exists on both entry paths. Only if the
...
- **Code path given** (from Turn 1's open-context prompt, or ask now: "Where's
