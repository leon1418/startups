---
_phase: turn1
_title: "Turn 1 — Entry Point + Background"
_init: true
_input: workspace
_assemble:
  _file: phases/turn1/turn1-assemble.md
_produces:
  - .phase-status.json
...
_postconditions:
  ...
  - _validate_json: .phase-status.json
    _on_failure: _halt_and_inform
  - _assert: "the .phase-status.json written by Step 5 has entry_point and audience set, turn1 = completed, and all later phases set to pending or skipped per the entry point"
    _on_failure: _halt_and_inform
---
