...
def test_skill_md_rows():
    ...
    # Row order: temporal_poc must be evaluated before temporal_worker.
    assert text.find("`temporal_poc`") < text.find("`temporal_worker` | `entry_point"), \
        "temporal_poc row must precede the temporal_worker row"
    # Main poc row no longer AgentCore-gated.
    poc_row = re.search(r"\| `poc` \|(.*?)\| Load", text)
    assert poc_row and "== agentcore" not in poc_row.group(1)
