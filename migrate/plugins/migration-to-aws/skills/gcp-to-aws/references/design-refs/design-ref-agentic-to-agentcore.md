...
- **AWS-native deployment:** First-class deployment on AgentCore Runtime with microVM isolation, 8-hour sessions (default serverless runtime; EC2-based runtime instances now support sessions up to 14 days), auto-scaling
...
- Up to 8-hour session duration for long-running agent tasks on the default serverless microVM-based AgentCore runtime; for longer-running agents, EC2-based runtime instances now support session durations of up to 14 days.
...
| `very_long` (1hr+)                 | AgentCore Runtime with session chaining. If using the default microVM runtime, break into sub-tasks if > 8 hours; alternatively, use EC2-based runtime instances which support sessions up to 14 days without chaining.            |
