...
- **AWS-native deployment:** First-class deployment on AgentCore Runtime with microVM isolation, 8-hour sessions (serverless) or up to 14 days with new runtime instances, auto-scaling
...
- Up to 8-hour session duration for serverless AgentCore Runtime; up to 14 days with new persistent runtime instances
...
| `very_long` (1hr+)                 | AgentCore Runtime with session chaining (serverless: 8h cap) or runtime instances (up to 14 days) for very long tasks; break into sub-tasks if exceeding the chosen deployment's cap. |
